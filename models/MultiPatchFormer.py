...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.base_stride = min(self.stride1, self.stride2, self.stride3, self.stride4)
        self.patch_num1 = math.ceil(self.seq_len / self.base_stride)
    ...
    def pad_to_target(self, x, target_len):
        B, C, L = x.shape
        if L < target_len:
            pad = target_len - L
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        P = self.patch_num1
        # 每个 scale 对齐 padding
        x_i_p1 = self.pad_to_target(x_i, P * self.stride1 + self.patch_len1 - self.stride1)
        x_i_p2 = self.pad_to_target(x_i, P * self.stride2 + self.patch_len2 - self.stride2)
        x_i_p3 = self.pad_to_target(x_i, P * self.stride3 + self.patch_len3 - self.stride3)
        x_i_p4 = self.pad_to_target(x_i, P * self.stride4 + self.patch_len4 - self.stride4)
