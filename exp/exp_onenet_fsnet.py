...
class Exp_TS2VecSupervised(Exp_Basic):
    def __init__(self, args):
        ...
        self.count = 0
        if self.individual:
            self.decision = MLP(n_inputs=args.pred_len * 3, n_outputs=1, mlp_width=32, mlp_depth=3, mlp_dropout=0.1,
            ...
            self.decision = MLP(n_inputs=(args.c_out * args.pred_len) * 3, n_outputs=1, mlp_width=32, mlp_depth=3,
                                mlp_dropout=0.1, act=nn.Tanh()).to(self.device)
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            print(
                "Epoch: {0}, Steps: {1} | Train Loss: {2:.4f} Vali Loss: {3:.4f} Test Loss:  {4:.4f} loss_ws:  {5:.4f} loss_bias:  {6:.4f}".format(
    ...
    def test(self, setting,test=0):
        if test:
            ...
            self.model.load_state_dict(torch.load(
                os.path.join('./ckpt/all_models', f"{self.args.model}_{self.args.trans_from}_{self.args.pred_len}",
                             'checkpoint.pth')))
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(tqdm(test_loader)):
            ...
            mapes.append(mape)
            mspes.append(mspe)
            # mae_reverse, mse_reverse, rmse_reverse, mape_reverse, mspe_reverse = metric(pred_reverse.numpy(),
            #                                                                             true_reverse.numpy())
        ...
        # MAE_reverse, MSE_reverse, RMSE_reverse, MAPE_reverse, MSPE_reverse = cumavg(maes_reverse), cumavg(
        #     mses_reverse), cumavg(rmses_reverse), cumavg(mapes_reverse), cumavg(mspes_reverse)
        # mae_reverse, mse_reverse, rmse_reverse, mape_reverse, mspe_reverse = MAE_reverse[-1], MSE_reverse[-1], \
        #     RMSE_reverse[-1], MAPE_reverse[-1], MSPE_reverse[-1]
    ...
    def _process_one_batch(self, dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark, mode='train'):
        ...
        x = batch_x.float().to(
            self.device)  # torch.cat([batch_x.float(), batch_x_mark.float()], dim=-1).to(self.device)
        ...
        if self.individual:
            ...
            loss1 = F.sigmoid(self.weight)  # the weight of cross-time forecaster
        ...
        l1, l2 = criterion(y1, rearrange(batch_y, 'b t d -> b (t d)')), criterion(y2, rearrange(batch_y,
                                                                                                'b t d -> b (t d)'))
        ...
        return [y1, y2], rearrange(batch_y,
                                   'b t d -> b (t d)'), loss_w.detach().cpu().item(), loss_bias.detach().cpu().item()
